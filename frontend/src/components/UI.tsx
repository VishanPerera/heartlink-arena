...
export function PrototypeNav({ screen, goTo, lockNavigation = false }: { screen: string; goTo: (s: string) => void; lockNavigation?: boolean }) {
  return (
      ...
      <div className="proto-panel">
        ...
        {NAV.map(s => (
          <button key={s.id} onClick={() => goTo(s.id)}
            disabled={lockNavigation && s.id !== 'login'}
            className={`proto-btn${screen === s.id ? ' current' : ''}${lockNavigation && s.id !== 'login' ? ' disabled' : ''}`}>
            {s.icon} {s.label}
          </button>
        ))}
      </div>
